...
namespace CajeroAutomatico.Entities
{
    public class ATM
    {
        ...
        public  void Start(BaseDatos baseDatos, int nroCuenta)
        {
            String data;
            ...
            while (true)
            {
                Console.WriteLine("Ingrese El numero de la opcion que desea realizar y Presione Enter");
                data = Console.ReadLine();
                if (data.CompareTo("3") == 0)
                {
                    break;
                }
                else if (data.CompareTo("1") == 0)
                {
                    lib.iniciarDeposito(baseDatos, nroCuenta);
                    break;
                }
                else if (data.CompareTo("2") == 0)
                {
                    lib.iniciarRetiro(baseDatos, nroCuenta);
                    break;
                }
